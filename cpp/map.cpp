...
//L'index 0 est la position du dernier élément et la recherche commence à parti de la position 1 de la chaine.
//Les curseurs prennent comme valeur la position des espaces.
void getInts(string line, int result[7]){
    int curs1 = 1, curs2;
    int i = 1; //Compteur: nb d'entiers rangés dans le tableau
    
    while(line.find_first_of(' ', curs1+1) != string::npos){
...
Map::Map(string src){  
    ifstream fichier(("maps/"+src+".map").c_str(), ios::in);
    assert(fichier);
    
    string line;
    int result[7];

    getline(fichier, line); //Saute une ligne
   ...
   do{
        switch(line[0]){
            case 'P':
                getInts(line, result);
                pnt[result[1]].x = result[1];
                pnt[result[1]].y = result[2];
                break;
...
float Map::findWall(Player player, float angle){
    ...
    do {
        ...
        if(e.type == 0){
            ...
            int xp = player.pos.x, yp = player.pos.y, x1 = e.A.x, y1 = e.A.y, x2 = e.B.x, y2 = e.B.y;
            int x = -(cos(angle)*(-x1*y2+y1*x2+yp*(x1-x2))+sin(angle)*xp*(x2-x1))/(cos(angle)*(y2-y1)+sin(angle)*(x1-x2));
            int y = -(sin(angle)*(-x1*y2+y1*x2+xp*(y2-y1))+cos(angle)*yp*(y1-y2))/(cos(angle)*(y2-y1)+sin(angle)*(x1-x2));
...
//--------------------------------------------------------------------//
=======
bool isInFace(Player p){
    return true;
}

float findWall(Player p){
    float dist; //Distance au mur
    
    //On lance le rayon et on passe de face en face jusqu'à ce que l'on se cogne contre un mur
    bool bang_le_mur = true;
    do{
        bang_le_mur = false;
    } while(bang_le_mur);
